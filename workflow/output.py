...
from .hooks import _default_confirm
...
def run_review_diff_stage(
    ...
    *,
    ...
    if runtime.config.interactive:
        ...
        try:
            choice = confirm_apply_changes("Apply these changes?", default=True)
        except EOFError:
            choice = False
        if not choice:
...
def run_applying_stage(runtime: WorkflowRuntime, event_bus: EventBus) -> WorkflowState:
    ...
    affected = apply_changes(
        runtime.target_dir, runtime.all_changes, event_bus=event_bus
    )
    runtime.final_summary = runtime.final_summary or approved_plan.summary
    runtime.affected_files = affected
    if runtime.config.auto_commit and affected:
        return WorkflowState.COMMITTING
